...
export default function Sidebar({ active, onTabChange, onAddClick }: SidebarProps) {
  return (
    <aside className="hidden w-64 flex-shrink-0 flex-col border-r border-page bg-card p-5 lg:flex">
      ...
      <button
        type="button"
        onClick={onAddClick}
        className="mt-8 flex items-center justify-center gap-2 rounded-pill bg-category-yemek py-3 text-sm font-bold text-white shadow-btn-primary active:scale-95"
      >
  ...
  return (
    <button
      ...
      className={`flex items-center gap-2.5 rounded-pill px-3 py-2.5 text-left text-sm font-semibold ${
        active ? "bg-page text-ink" : "text-muted"
      }`}
    >
      {icon}
      {label}
