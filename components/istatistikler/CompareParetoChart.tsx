...
interface CompareParetoChartProps {
  ...
  periodALabel: string;
  periodBLabel: string;
  periodADateAt: (pct: number) => string;
  periodAGranularityLabel: string;
}

export default function CompareParetoChart({
  data,
  periodALabel,
  periodBLabel,
  periodADateAt,
  periodAGranularityLabel,
}: CompareParetoChartProps) {
  if (data.length === 0) {
  ...
  return (
    <>
      <ResponsiveContainer width="100%" height={240}>
        <ComposedChart data={data} margin={{ top: 4, right: 12, left: -22, bottom: 0 }}>
          ...
          <YAxis
            ...
            tickFormatter={(v) => periodADateAt(Number(v))}
            tick={CHART_TICK}
            axisLine={false}
            tickLine={false}
          />
      ...
      <p className="mt-1 text-[11px] font-medium text-muted">
        Sağ eksen: {periodALabel} döneminde {periodAGranularityLabel} çözünürlük
      </p>
    </>
  );
}
